...
Never put a token directly in argv, logs, prompts, or committed files. `--force` spends another Tier 1 request. Until metadata probing is implemented, a normal pull for an already cached lineage returns `metadata_unavailable` instead of silently spending another request. This CLI guard is defense in depth; the Agent must still perform the offline preflight and obtain the per-operation authorization described above.
